...
exports.POST = POST;
exports.POST = POST;
exports.useCommentsAnalysis = useCommentsAnalysis;
const server_1 = require("next/server");
const insightiq_1 = require("../../lib/insightiq");
// Initialize InsightIQ client
const insightiq = new insightiq_1.InsightIQClient({
...
async function POST(request) {
    try {
        ...
        if (error instanceof Error && 'status' in error) {
            ...
            return server_1.NextResponse.json({ error: apiError.message, details: apiError.details }, { status: apiError.status || 500 });
        }
...
async function GET(request, { params }) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to get insights' }, { status: 500 });
    }
}
...
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to start analysis' }, { status: 500 });
    }
}
...
async function POST(request) {
    try {
        const body = await request.json();
        const { work_platform_id, keyword, hashtag, mention, items_limit = 50 } = body;
        if (!work_platform_id || (!keyword && !hashtag && !mention)) {
            return server_1.NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
        }
        let search;
        if (keyword) {
            search = await insightiq.socialListening.searchByKeyword(work_platform_id, keyword, { itemsLimit: items_limit });
        }
        else if (hashtag) {
            search = await insightiq.socialListening.searchByHashtag(work_platform_id, hashtag, { itemsLimit: items_limit });
        }
        else if (mention) {
            search = await insightiq.socialListening.searchByMention(work_platform_id, mention, { itemsLimit: items_limit });
        }
        return server_1.NextResponse.json({
            success: true,
            search_id: search.id,
        ...
        return server_1.NextResponse.json({ error: 'Failed to start search' }, { status: 500 });
    }
}
...
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to process webhook' }, { status: 500 });
    }
}
...
function useCommentsAnalysis() {
    const [state, setState] = (0, react_1.useState)({
        loading: false,
        data: null,
        error: null
    });
    const startAnalysis = (0, react_1.useCallback)(async (data) => {
    ...
    const getResults = (0, react_1.useCallback)(async (analysisId) => {
        try {
